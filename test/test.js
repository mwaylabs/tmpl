...
describe('tmpl', function() {
    ...
    it('check stickitAttribute with false', function() {
        tmplOptions = {
            useStickitAttribute: false
        };
        tmplData = {f1: 'value_f1'};
        assert.equal(_.tmpl('<div></div>', null, tmplOptions)(tmplData), '<div></div>');
        assert.equal(_.tmpl('<div class="a"></div>', null, tmplOptions)(tmplData), '<div class="a"></div>');
        assert.equal(_.tmpl('<div><%= f1 %></div>', null, tmplOptions)(tmplData), '<div>value_f1</div>');
        assert.equal(_.tmpl('<div class="a"><%= f1 %></div>', null, tmplOptions)(tmplData), '<div class="a">value_f1</div>');
        assert.equal(_.tmpl('<div class="<%= f1 %>"><%= f1 %></div>', null, tmplOptions)(tmplData), '<div class="value_f1">value_f1</div>');
        assert.equal(_.tmpl('<div class="<%= f1 %>"><%= f2 %></div>', null, tmplOptions)(tmplData), '<div class="value_f1"></div>');
        assert.equal(_.tmpl('<div class="<%= f1 %>"><%= f1 %><%= f2 %></div>', null, tmplOptions)(tmplData), '<div class="value_f1">value_f1</div>');
        assert.equal(_.tmpl('<div class="<% if(f1){ %>a<% } %>"><%= f1 %></div>', null, tmplOptions)(tmplData), '<div class="a">value_f1</div>');
        assert.equal(_.tmpl('<div id="<%= f1 %>" class="<%= f2 %>"></div>', null, tmplOptions)(tmplData), '<div id="value_f1" class=""></div>');

    });

    it('switch between default and custom stickit attribute but ignore it', function() {

        tmplOptions = {
            stickitAttribute: 'data-mystickit',
            useStickitAttribute: false
        };
        tmplData = {f1: 'value_f1', f2: 'value_f2'};
        assert.equal(_.tmpl('<div></div>', null, tmplOptions)(tmplData), '<div></div>');
        ...
        assert.equal(_.tmpl('<div class="a"><%= f1 %></div>', null, tmplOptions)(tmplData), '<div class="a">value_f1</div>');
        assert.equal(_.tmpl('<div class="<%= f1 %>"><%= f1 %></div>', null, tmplOptions)(tmplData), '<div class="value_f1">value_f1</div>');
        assert.equal(_.tmpl('<div class="<%= f1 %>"><%= f2 %></div>', null, tmplOptions)(tmplData), '<div class="value_f1">value_f2</div>');
        assert.equal(_.tmpl('<div class="<%= f1 %>"><%= f1 %><%= f2 %></div>', null, tmplOptions)(tmplData), '<div class="value_f1">value_f1value_f2</div>');
        assert.equal(_.tmpl('<div class="<% if(f1){ %>a<% } %>"><%= f1 %></div>', null, tmplOptions)(tmplData), '<div class="a">value_f1</div>');
        assert.equal(_.tmpl('<div id="<%= f1 %>" class="<%= f2 %>"></div>', null, tmplOptions)(tmplData), '<div id="value_f1" class="value_f2"></div>');
        assert.equal(_.tmpl('<div id="<%= f1 %>" class="<%= f1 %><%= f2 %>"></div>', null, tmplOptions)(tmplData), '<div id="value_f1" class="value_f1value_f2"></div>');
        assert.equal(_.tmpl('<div><div id="<%= f1 %>" class="<%= f1 %><%= f2 %>"></div></div>', null, tmplOptions)(tmplData), '<div><div id="value_f1" class="value_f1value_f2"></div></div>');
        assert.equal(_.tmpl('<div><div id="<%= f1 %>" class="<%= f1 %><%= f2 %>"><div><%= f1 %></div></div></div>', null, tmplOptions)(tmplData), '<div><div id="value_f1" class="value_f1value_f2"><div>value_f1</div></div></div>');
        assert.equal(_.tmpl('<div><div class="<%= f1 %>" contenteditable="true"><%= f1 %></div><div contenteditable="true"><%= f2 %></div></div>', null, tmplOptions)(tmplData), '<div><div class="value_f1" contenteditable="true">value_f1</div><div contenteditable="true">value_f2</div></div>');
        assert.equal(_.tmpl('<input type="text" name="input" value="<%= f1 %>" />', null, tmplOptions)(tmplData), '<input type="text" name="input" value="value_f1" />');
        assert.equal(_.tmpl('<input placeholder="<%= f1 %>" type="text" name="input" value="<%= f1 %>" />', null, tmplOptions)(tmplData), '<input placeholder="value_f1" type="text" name="input" value="value_f1" />');
    });
